...
class LevelTrackerStream(levelsChannel: TextChannel)(implicit ex: ExecutionContextExecutor, mat: Materializer) extends StrictLogging {
  ...
  case class CharKey(char: String, time: ZonedDateTime, level: Double)
  ...
  private lazy val getCharacterData = Flow[WorldResponse].mapAsync(1) { worldResponse =>
    ...
    recentOnline.addAll(online.map(i => CharKey(i, now, i.char.characters.character.level))) // ...and add them again, with an updated online time
  ...
  private lazy val scanForLevels = Flow[Set[CharacterResponse]].mapAsync(1) { characterResponses =>
    ...
    val newLevels = characterResponses.flatMap { char =>
      val levels: List[Levels] = List(char.characters.character.last_login, char.characters.character.level)
      levels.flatMap { level =>
        val levelTime = ZonedDateTime.parse(level.time)
        val levelAge = java.time.Duration.between(levelTime, now).getSeconds
        val charLevel = CharKey(char.characters.character.name, levelTime, level.level)
