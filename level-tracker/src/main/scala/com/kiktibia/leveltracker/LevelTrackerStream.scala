...
class LevelTrackerStream(levelsChannel: TextChannel)(implicit ex: ExecutionContextExecutor, mat: Materializer) extends StrictLogging {
  ...
  private lazy val getCharacterData = Flow[WorldResponse].mapAsync(1) { worldResponse =>
    ...
    recentOnline.addAll(online.map(i => (i.name, i.level)).toList) // ...and add them again, with an updated online time
        ...
        if (olName == name){
          ...
          for (l <- recentLevels
            ...
            if olName == l.char){
              ...
              val recentLogin = l.lastLogin.get
              val currentLogin = sheetLogin.get
              if (ZonedDateTime.parse(recentLogin).isBefore(ZonedDateTime.parse(currentLogin))) {
                println(recentLogin)
