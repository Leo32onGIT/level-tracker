      ...
      onlineLevel.flatMap { case (olName, olLevel) =>
        if (olName == name){
          ...
          for (l <- recentLevels
            ...
            if olName == l.char){
              println(s"Online /w Level Entry: $olName, $olLevel, ${sheetLogin.getOrElse("2022-01-01T01:00:00Z")}\nRecent Level: ${l.char}, ${l.char}, ${l.lastLogin}")
          ...
          //if (olLevel > sheetLevel && !recentLevels.map(_.level).contains(olLevel)) {
