...
class LevelTrackerStream(levelsChannel: TextChannel)(implicit ex: ExecutionContextExecutor, mat: Materializer) extends StrictLogging {
  ...
  private lazy val scanForLevels = Flow[Set[CharacterResponse]].mapAsync(1) { characterResponses =>
    ...
    val newLevels = characterResponses.flatMap { char =>
			...
			val levels: List[CharKey] = recentOnline.map(i => List(CharKey(i.char, Levels(i.level.time, i.level.level)))).toSet
			...
			val charLevel = CharKey(char.characters.character.name, Levels(now.toString, levels.level.level))
			if (levels.level.level > charSheet && !recentLevels.contains(charLevel)){
				recentLevels.add(charLevel)
				Some(CharLevel(char, Levels(now.toString, levels.level.level)))
			}
